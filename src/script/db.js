...
function card({ id, airline, type, route, price, stops, departureTime }) {
  ...
  innerCard.innerHTML = `<div id="fCard" class="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-4">
        <div
          class="flex flex-col md:flex-row justify-between border-2 border-neutral-400 border-dashed rounded-2xl p-6 md:p-10 gap-6">
          <div class="flex items-center gap-3">
            <div class="font-semibold text-xl md:text-2xl">${airline}</div>
            <div class="text-xs md:text-sm text-neutral-600 px-2 py-0.5 bg-neutral-100 rounded-full">${type}</div>
          </div>
          <div class="flex flex-col items-center text-center">
            <div class="text-base md:text-lg font-bold">${route.from.city}</div>
            <div class="text-xs md:text-sm text-gray-500">${route.from.code}</div>
          </div>
          <div class="flex flex-col items-center text-center">
            <div class="text-xs md:text-sm text-gray-500">${stops}</div>
            ...
            <div class="text-xs md:text-sm text-gray-500">${departureTime}</div>
          </div>
          <div class="flex flex-col items-center text-center">
            <div class="text-base md:text-lg font-bold">${route.to.city}</div>
            <div class="text-xs md:text-sm text-gray-500">${route.to.code}</div>
          </div>
        </div>
        <div class="border-2 border-dashed border-neutral-400 rounded-2xl p-6 md:p-10 flex flex-col justify-center">
          <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div class="flex flex-col text-center sm:text-left">
              <div class="font-bold text-lg md:text-xl text-neutral-900">
            ...
            <button id="add-btn-${id}" onclick="handleAddToCart(${id})"
              class="cursor-pointer w-full sm:w-auto px-6 py-3 bg-neutral-900 text-white text-sm md:text-base rounded-2xl transition-colors duration-300 hover:bg-black">
              Add To Cart
            </button>
          </div>
        </div>
